...
[[ -f "$root/composer.json" && -f "$root/composer.lock" ]] || fail 'run composer install first; composer.json and composer.lock are required.'
work="$(mktemp -d)"
...
' "$work/direct.json" >/dev/null || fail 'direct Kumwe dependencies must use exact stable versions, without aliases or ranges.'
...
' "$work/resolved.json" >/dev/null || fail 'Composer lock must contain each exact required Kumwe package once; replace/provide aliases are not a release.'
jq -e 'length == (map(.name) | unique | length)' "$work/resolved.json" >/dev/null || fail 'duplicate Kumwe packages in Composer lock.'
...
[[ -f "$readiness" ]] || fail 'resources/release-readiness.json must locate external attestations for resolved Kumwe dependencies.'
...
while IFS= read -r locked; do
  ...
  [[ "$version" =~ ^(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)$ ]] || fail "$name resolved to a non-stable version: $version"
  tag="v$version"
  jq -e --arg name "$name" '
    .source.type == "git" and
  ...
  gh api --method GET "repos/$name/releases/tags/$tag" > "$work/release.json" || fail "cannot retrieve $name $tag release."
  jq -e --arg tag "$tag" '
  ...
  jq -e --arg tag "$tag" '.ref == ("refs/tags/" + $tag)' "$work/tag.json" >/dev/null || fail "$name returned a different tag."
  for ((depth=0; depth<6; depth++)); do
    ...
    [[ "$object_type" == tag && "$depth" -lt 5 ]] || fail "$name tag must resolve to a commit within five annotated tags."
    gh api --method GET "repos/$name/git/tags/$object_sha" > "$work/next-tag.json" || fail "cannot peel $name annotated tag."
  ...
  ' "$readiness" >/dev/null || fail "$name $version needs a digest-pinned external JSON release attestation at a full commit."
  ...
  gh api --method GET "repos/$evidence_repo/contents/$evidence_path?ref=$evidence_commit" > "$work/content.json" || fail "cannot retrieve external evidence for $name."
  jq -e --arg path "$evidence_path" '.type == "file" and .path == $path and .encoding == "base64" and (.content | type == "string")' "$work/content.json" >/dev/null || fail "$name attestation must be a repository file."
  jq -r .content "$work/content.json" | base64 --decode > "$work/attestation.json" || fail "$name attestation encoding is invalid."
  actual_sha="$(sha256sum "$work/attestation.json")"
  [[ "${actual_sha%% *}" == "$evidence_sha" ]] || fail "$name external attestation digest does not match its reviewed coordinate."
  jq -e --arg name "$name" --arg version "$version" --arg tag "$tag" --arg commit "$commit" \
    ...
    (.manifests_and_corpora | type == "array" and length > 0 and all(.[]; (.path | nonempty) and (.sha256 | digest))) and
  ...
  ' "$work/attestation.json" >/dev/null || fail "$name external attestation must independently verify this exact release, artifacts, registry, manifests and clean consumer without gaps."
  ...
  gh api --method GET "repos/$name/actions/runs/$run_id" > "$work/run.json" || fail "cannot retrieve $name release workflow evidence."
  ...
  ' "$work/run.json" >/dev/null || fail "$name release-on-record workflow must have succeeded for the exact tagged commit."
...
if [[ -n "$output" ]]; then
  jq '{schema: "kumwe-dependency-verification/v1", status: "verified", dependencies: .}' "$work/evidence.json" > "$output"
fi
