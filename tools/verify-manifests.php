...
const BUSINESS_DEFINITION_NAMESPACE = 'Kumwe\\BusinessDefinition\\';
const BUSINESS_DEFINITION_PUBLIC_API = 'resources/public-api/v1.json';
const BUSINESS_DEFINITION_CAPABILITIES = 'resources/capabilities/v1.json';
const BUSINESS_DEFINITION_SERVICE_MAP = 'resources/service-map/v1.json';
const BUSINESS_DEFINITION_CHANGELOG = 'CHANGELOG.md';
const BUSINESS_DEFINITION_API_DOCUMENT = 'docs/public-api.md';
const BUSINESS_DEFINITION_CAPABILITY_PREFIX = 'business-definition.';
const BUSINESS_DEFINITION_LIFETIMES = ['shared', 'non-shared', 'request-supplied'];
...
function businessDefinitionRegisterAutoloader(): void
{
    spl_autoload_register(static function (string $class): void {
        ...
        $relative = substr($class, strlen(BUSINESS_DEFINITION_NAMESPACE));
        $path = BUSINESS_DEFINITION_SOURCE . '/' . str_replace('\\', '/', $relative) . '.php';
        if (is_file($path)) {
            require $path;
        }
    });
...
function businessDefinitionReflectedSymbols(): array
{
    ...
    $iterator = new RecursiveIteratorIterator(
        new RecursiveDirectoryIterator(BUSINESS_DEFINITION_SOURCE, FilesystemIterator::SKIP_DOTS),
    );
...
function businessDefinitionPublicApiManifest(array $symbols, string $release): array
{
    ...
    return [
        ...
        'package' => BUSINESS_DEFINITION_PACKAGE,
        'release' => $release,
        'namespace' => BUSINESS_DEFINITION_NAMESPACE,
...
function businessDefinitionVerifyCapabilities(array $exported, string $release): int
{
    ...
    foreach ($capabilities as $capability) {
        ...
        if (
            ...
            || !str_starts_with($id, BUSINESS_DEFINITION_CAPABILITY_PREFIX)
        ) {
            throw new RuntimeException($file . ' carries a capability id outside the business-definition.* form.');
        }
        if (isset($seen[$id])) {
...
function businessDefinitionVerifyServiceMap(array $exported, string $release): string
{
    ...
    foreach ($factories as $factory) {
        ...
        if (!in_array($factory['lifetime'] ?? null, BUSINESS_DEFINITION_LIFETIMES, true)) {
    ...
    foreach ($keys as $entry) {
        ...
        if (!is_string($key) || preg_match('/^kumwe\.business-definition(\.[A-Za-z0-9_.-]+)+$/', $key) !== 1) {
            throw new RuntimeException($file . ' carries a configuration key outside kumwe.business-definition.*.');
        }
