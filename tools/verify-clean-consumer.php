...
$workspace = rtrim(sys_get_temp_dir(), '/\\') . '/kumwe-business-definition-consumer-' . bin2hex(random_bytes(6));
...
consumerRun(
    [
        ...
        '--format=zip',
        '--dir=' . $distDirectory,
        '--file=kumwe-business-definition',
    ],
    $workspace,
);
$archive = $distDirectory . '/kumwe-business-definition.zip';
if (!is_file($archive)) {
...
$installed = $consumer . '/vendor/kumwe/business-definition';
...
consumerRun([PHP_BINARY, $installed . '/examples/definition.php', $autoload], $workspace);
...
foreach (array_keys($classmap) as $symbol) {
    if (
        ...
        || str_starts_with($symbol, 'Kumwe\\BusinessDefinition\\Tests\\')
    ) {
